...
def train_classifier_model(train_dataset, test_dataset, path, C=2**np.arange(-8, 1).astype(np.float), seed=42, penalty="l1"):
    ...
    for i, c in enumerate(C):
        logreg_model = LogisticRegression(C=c, penalty=penalty, random_state=seed+i, solver="saga", multi_class='multinomial')
        logreg_model.fit(trX, trY)

        # score = logreg_model.score(teX, teY)
        y_pred = logreg_model.predict(teX)
        score = balanced_accuracy_score(teY, y_pred)
        scores.append(score)

    c = C[np.argmax(scores)]

    sent_classfier = LogisticRegression(C=c, penalty=penalty, random_state=seed+len(C), solver="saga", multi_class='multinomial', max_iter=100)
    sent_classfier.fit(trX, trY)

    # score =  sent_classfier.score(teX, teY) * 100
    y_pred = sent_classfier.predict(teX)
    score = balanced_accuracy_score(teY, y_pred) * 100
    ...
    # Plot results
    # plot_results.plot_weight_contributions(sent_classfier.coef_)
    # plot_results.plot_logistcs(trX, trY, sentneuron_ixs)
    print(type(sentneuron_ixs))
