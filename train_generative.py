...
def build_vocabulary(train_vocab, test_vocab, path):
    ...
    with open(os.path.join(path, "vocabulary_dict.json"), "w") as f:
...
def train_generative_model(model, train_dataset, test_dataset, model_epochs, learn_rate, path):
    ...
    checkpoint_prefix = os.path.join(SAVE_CHECKPOINTS, "generative_ckpt_{epoch}")
